...
class Student:
    ...
    def get_exam_result(self, course_name):
        ...
        if len(self._course_exams[course_name]) != 0:
            aver_res = sum(
                self._course_exams[course_name][i] for i in range(len(self._course_exams[course_name]))) / len(
                self._course_exams[course_name])
        else:
            aver_res = -999
    ...
    def get_test_result(self, course_name):
        ...
        if len(self._course_tests[course_name]) != 0:
            aver_res = sum(
                self._course_tests[course_name][i] for i in range(len(self._course_tests[course_name]))) / len(
                self._course_tests[course_name])
        else:
            aver_res = -999
        return aver_res
...
def print_res(stu, type_, course):
    if type_ == "exam":
        print(
            f'Студент {stu.get_full_name} имеет среднюю оценку за экзамены по предмету {course} = {stu.get_exam_result(course)}')
    if type_ == "test":
        print(
            f'Студент {stu.get_full_name} имеет среднюю оценку за тесты по предмету {course} = {stu.get_test_result(course)}')
...
print(
    f'Студент {stu1.get_full_name} имеет среднюю оценку  по экзаменам за все предметы = {stu1.get_total_exam_result()}')
